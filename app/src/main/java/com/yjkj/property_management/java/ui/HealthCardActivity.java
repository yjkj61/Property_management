...
import com.yjkj.property_management.java.adapter.HealthCardAdapter;
import com.yjkj.property_management.java.bean.AfBean;
import com.yjkj.property_management.java.bean.HealthBean;
import com.yjkj.property_management.java.bean.LastPhysicalExamination;
                ...
                if (response.body() != null) {
                    ...
                    if (bean.getCode() == 200){
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                list.add(new HealthBean("血压", bean.getData().getHHypotension() + "/" + bean.getData().getHHypertension(), 0));
                                list.add(new HealthBean("体温", bean.getData().getHTemperatureValue(), 0));
                                list.add(new HealthBean("血氧", bean.getData().getHBloodOxygenSaturation(), 0));
                                list.add(new HealthBean("脉搏", bean.getData().getHPulseRate(), 0));
                                list.add(new HealthBean("血糖", bean.getData().getHBloodSugarValue(), 0));
                                list.add(new HealthBean("尿酸", bean.getData().getHUricAcidValue(), 0));
                                list.add(new HealthBean("胆固醇", bean.getData().getHCholesterolContent(), 0));
//                        list.add(new HealthBean("血脂四项", bean.getData().getHHypotension() + "/" + bean.getData().getHHypertension(), 0));
